...
func UnpackBaseElement(src any, keepPointers bool) any {
	if src == nil {
		return nil
	}
	srcReflectValue := reflect.ValueOf(src)

	// process initial element
	childElem := srcReflectValue
	switch srcReflectValue.Kind() {
	case reflect.Ptr:
		if keepPointers {
			return src
		}
		childElem = srcReflectValue.Elem()
	case reflect.Interface:
		childElem = srcReflectValue.Elem()
	default: // not ptr or interface, we return the element directly
		return src
	}

	// process child element
	if !childElem.IsValid() {
		return nil
	}

	switch childElem.Kind() {
	case reflect.Ptr:
		if !childElem.IsNil() && !keepPointers {
			return UnpackBaseElement(childElem.Interface(), keepPointers)
		}
	case reflect.Interface:
		if !childElem.IsNil() {
			return UnpackBaseElement(childElem.Interface(), keepPointers)
